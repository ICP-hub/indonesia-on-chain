...
actor {
  ...
  public shared ({ caller }) func is_user_exist_byprincipal(calleby:Principal) : async Result.Result<Bool, Bool> {

    let is_authenticated = await Auth.auth_user(calleby);

    switch (is_authenticated) {
      case (#ok(value)) {
        switch (user_map.get(calleby)) {
          case (?user) {
            return #ok(value);
          };
          case (null) { return #err(false) }; // User not found
